...
//Convenience functions for generating a new AS3 class instance of b2SimplexCache
FREResult FRENewObjectFromb2SimplexCache(b2SimplexCache* obj, FREObject as3Object) {
	...
	const uint8_t* className = (const uint8_t*)"ca.digitalarchitect.box2dane.collision.b2SimplexCache";

	FREResult objInitResult = FRENewObject(className, 1, constructorArguments, &as3Object, &initException);

	if(objInitResult != FRE_OK) {
		FREError("Error creating b2SimplexCache object in function FRENewObjectFromb2SimplexCache");
	}

	return objInitResult;
}

FREResult FRENewb2SimplexCache(FREObject as3Object) {
	...
	if(objInitResult != FRE_OK) {
		FREError("Error creating b2SimplexCache object in function FRENewb2SimplexCache");
	}
