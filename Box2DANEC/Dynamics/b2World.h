...
//Convenience functions for generating a new AS3 class instance of b2World
FREResult FRENewObjectFromb2World(b2World* obj, FREObject as3Object) {
	...
	const uint8_t* className = (const uint8_t*)"ca.digitalarchitect.box2dane.dynamics.b2World";

	FREResult objInitResult = FRENewObject(className, 1, constructorArguments, &as3Object, &initException);

	if(objInitResult != FRE_OK) {
		FREError("Error creating b2World object in function FRENewObjectFromb2World");
	}

	return objInitResult;
}

FREResult FRENewb2World(FREObject as3Object) {
	...
	if(objInitResult != FRE_OK) {
		FREError("Error creating b2World object in function FRENewb2World");
	}
